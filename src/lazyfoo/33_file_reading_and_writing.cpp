...
bool loadMedia()
{
	...
	//Open the font
	gFont = TTF_OpenFont( "img/33_file_reading_and_writing/lazy.ttf", 28 );
	if( gFont == NULL )
	{
	...
	SDL_RWops* file = SDL_RWFromFile( "img/33_file_reading_and_writing/nums.bin", "r+b" );
	...
	if( file == NULL )
	{
		...
		file = SDL_RWFromFile( "img/33_file_reading_and_writing/nums.bin", "w+b" );
		if( file != NULL )
		{
...
void close()
{
	...
	SDL_RWops* file = SDL_RWFromFile( "img/33_file_reading_and_writing/nums.bin", "w+b" );
	if( file != NULL )
	{
		//Save data
		for( int i = 0; i < TOTAL_DATA; ++i )
